...
class CustomCryptoPostgresHook(BaseHook):
    ...
    def bulk_load(self, table_name, file_name, delimiter: str, is_header: bool, is_replace: bool):
        ...
        # Log the size of the dataframe after dropping duplicates
        self.log.info(f'파일에서 중복 제거 후 데이터 건수: {len(file_df)}')

        # Check and remove existing data from PostgreSQL
        if not is_replace:
            ...
            # Log the size of the dataframe after removing existing dates
            self.log.info(f'중복 제거 후 데이터 건수: {len(file_df)}')
        
        # Log the size of the dataframe before loading to SQL
